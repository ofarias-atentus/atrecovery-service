"""Stage 3 tests: object grants gate routine/resource reads; admin manages grants."""
...
async def test_seed_grants_operator_hello_and_moto(client):
    ...
    routines = (await client.get("/api/v1/routines", headers=op)).json()
    assert any(t["name"] == "hello.py" for t in routines)
    hello = next(t for t in routines if t["name"] == "hello.py")
    assert (await client.get(f"/api/v1/routines/{hello['id']}", headers=op)).status_code == 200
    fetch = await client.get(f"/api/v1/routines/{hello['id']}/fetch", headers=op)
    content = fetch.json()["content"]
    source = content["source"] if isinstance(content, dict) else content
    assert fetch.status_code == 200 and "hello from routine" in source
...
async def test_ungranted_routine_denied_then_direct_grant_allows(client):
    ...
    # filtered from list, 403 on detail + fetch (operator HAS routine:view/use codes)
    assert all(t["id"] != tpl["id"] for t in (await client.get("/api/v1/routines", headers=op)).json())
    assert (await client.get(f"/api/v1/routines/{tpl['id']}", headers=op)).status_code == 403
    assert (await client.get(f"/api/v1/routines/{tpl['id']}/fetch", headers=op)).status_code == 403
    # view-only direct user grant: GET ok, fetch still 403
    operator_id, _ = await _ids(client, admin)
    g = (
        await client.post(
            "/api/v1/grants/routines",
            headers=admin,
            json={"routine_id": tpl["id"], "principal_type": "user",
                  "principal_id": operator_id, "can_view": True, "can_use": False},
        )
    ).json()
    assert (await client.get(f"/api/v1/routines/{tpl['id']}", headers=op)).status_code == 200
    assert (await client.get(f"/api/v1/routines/{tpl['id']}/fetch", headers=op)).status_code == 403
    ...
    assert (await client.get(f"/api/v1/routines/{tpl['id']}", headers=op)).status_code == 200
    assert (await client.get(f"/api/v1/routines/{tpl['id']}/fetch", headers=op)).status_code == 200
    # deleting the grant revokes access again
    grants = (await client.get("/api/v1/grants/routines", headers=admin)).json()
    gid = next(x["id"] for x in grants if x["routine_id"] == tpl["id"])
    await client.delete(f"/api/v1/grants/routines/{gid}", headers=admin)
    assert (await client.get(f"/api/v1/routines/{tpl['id']}", headers=op)).status_code == 403
...
async def test_grant_management_guards(client):
    ...
    hello = next(
        t for t in (await client.get("/api/v1/routines", headers=admin)).json() if t["name"] == "hello.py"
    )
    # operator cannot manage grants
    assert (
        await client.post(
            "/api/v1/grants/routines", headers=op,
            json={"routine_id": hello["id"], "principal_type": "role",
                  "principal_id": operator_role, "can_view": True},
        )
    ).status_code == 403
    assert (await client.get("/api/v1/grants/routines", headers=op)).status_code == 403
    ...
    assert (
        await client.post(
            "/api/v1/grants/routines", headers=admin,
            json={"routine_id": hello["id"], "principal_type": "user",
    ...
    assert (await client.delete("/api/v1/grants/routines/9999", headers=admin)).status_code == 404
...
async def test_superuser_bypass_and_missing_objects(client):
    ...
    assert any(t["id"] == tpl["id"] for t in (await client.get("/api/v1/routines", headers=admin)).json())
    assert (await client.get(f"/api/v1/routines/{tpl['id']}/fetch", headers=admin)).status_code == 200
    # 404 still wins over 403 for missing objects
    assert (await client.get("/api/v1/routines/9999", headers=op)).status_code == 404
