# POC Plan — FastAPI Template Management System
...
Build a PoC FastAPI application to **retrieve, maintain, and categorize Templates** (mandatory rename: `script` → `template` everywhere, including DB tables/columns, APIs, UI; only `type/category` data and `python` type remain as-is).
...
2. Template catalog with categories (start: `python`, plus e.g. `json`; extensible).
...
4. Permission system for operators over templates/resources (RBAC + object-level grants + resource groups).
...
9. Usage limits (one template → many limits), each usage mapped to beacon results.
10. Activity tracking at log level (e.g. most-used template, last fetch time — generic, extensible).
...
./
  ...
  app/
    ...
    db/
      ...
      seed.py            # demo seed (admin/operator, categories, template, resource, group, etc.)
    models/
      __init__.py
      identity.py        # users, roles, permissions, user_roles, role_permissions, auth_identities
      catalog.py         # template_categories, templates
      resources.py       # resources, metadata_definitions, resource_metadata, resource_groups, members, assignments
      grants.py          # template_grants, resource_grants
      usage.py           # execution_modes, template_usages, usage_limits
    ...
    api/v1/
      auth.py, users.py, roles.py, categories.py, templates.py,
    ...
    templates/           # custom Jinja pages (login help, dashboard stub)
    static/
  tests/
    conftest.py
...
[template_categories] 1---* [templates] *---1 [users(created_by)]
...
[templates] 1---* [template_usages] *---1 [users(requested_by)]
[resources] 1---* [template_usages] (nullable)
[templates] 1---* [usage_limits]
[template_usages] 1---* [execution_results]
...
- `permissions(id PK, code UNIQUE e.g. template:view|template:use|resource:use|stats:view|admin:manage, description)`.
...
- `template_categories(id PK, name UNIQUE e.g. python|json, description, schema_hint JSON, is_active)`.
- `templates(id PK, name, version default 1, category_id FK→categories, content TEXT, input_schema JSON nullable, is_active BOOL, created_by FK→users, created_at, updated_at)` — UNIQUE(name, version). NOTE: no `script` naming anywhere.
...
- `template_grants(id PK, template_id FK, principal_type ENUM user|role|group, principal_id INT, can_view BOOL, can_use BOOL)`.
...
- `template_usages(id PK, template_id FK, resource_id FK nullable, requested_by FK→users, mode_id FK→modes, status ENUM pending|dispatched|running|done|failed, external_dispatch_id nullable (voucher ID from other system), schedule_at nullable, payload JSON, use_count INT default 1, created_at)`.
...
- `usage_limits(id PK, template_id FK, scope_type ENUM global|user|role|group, scope_id nullable, max_uses INT, window ENUM total|daily|monthly, is_active BOOL)` — one-to-many per template. Checked on fetch + usage creation.
...
- `execution_results/beacons(id PK, usage_id FK→template_usages, processor_id FK→processors, status e.g. ok|error|partial, result JSON, received_at)` — many beacons per usage (usage ↔ results = one-to-many).

**Tracking**

- `activity_logs(id PK, user_id FK nullable, action e.g. template.fetch|template.use|beacon.received|grant.changed, entity_type, entity_id nullable, meta JSON, ip nullable, created_at)` — append-only. Admin UI read-only (list/show, no edit/delete). Powers `most used template`, `last time user got template`, etc. via aggregation, not hardcoded columns.

**Statistics**

- `statistics_definitions(id PK, name UNIQUE e.g. most_used_template|last_fetch_by_user, source_type ENUM internal|external, query_config JSON, required_params JSON e.g. ["user_id"], required_permission_code FK→permissions.code, is_active BOOL)`.
...
- **RBAC enforcement:** `Depends(get_current_user)` → `Depends(require_permission("template:use"))` → `require_object_access(template_id, resource_id)` in service layer. Seed `admin:*`, `operator: template:view/use, resource:view/use, stats:view (gated per stat)`.
...
- **Logging:** `RequestIDMiddleware` (X-Request-ID), uvicorn + app logger to stdout (JSON-ish for PoC), `ActivityLogger.log(user, action, entity...)` called from template fetch, usage create, beacon ingest, grant/group changes.
...
- `/api/v1/templates` — list/get (needs `template:view` + grant), create/update/delete (admin), `GET /{id}/fetch` (needs `template:use`, checks limits, writes activity log, returns content).
- `/api/v1/resources`, `/metadata-definitions`, `/resources/{id}/metadata` (put/delete per key), `/resource-groups` + `/{id}/members` + `/{id}/assignments`.
- `/api/v1/grants/templates`, `/grants/resources` — admin manage.
...
### Stage 2 — Catalog: Categories, Templates, Resources, Metadata, Groups

- Entry: Stage 1 green.
- Tasks: models+schemas+routers for `template_categories/templates/resources/metadata_definitions/resource_metadata/resource_groups/members/assignments`; seed `python|json` categories, `hello.py` template, `ZY323S5GHW` resource + 7 metadata keys + group `lab-phones`; tests CRUD + metadata attach/detach + group membership.
- Exit: full catalog CRUD via API; seed data present.

### Stage 3 — Access Control (Grants + Group Auto-Assign)

- Entry: Stage 2 green.
- Tasks: `template_grants/resource_grants` models/schemas/routers; `services/rbac.py` evaluator + `require_object_access`; group→principal resolution; seed grants (operator can view/use lab group + hello template); tests allowed/denied matrix.
- Exit: operator without grant gets 403; with grant succeeds.

### Stage 4 — Usage Layer + Limits (direct/scheduler/voucher)

- Entry: Stage 3 green.
- Tasks: `execution_modes/template_usages/usage_limits` + `services/usage_svc.py` (limit check scopes global/user/role/group × windows total/daily/monthly); routers `execution-modes/usages` incl. `POST /usages` + `GET /{id}/status`; voucher `external_dispatch_id` handling; tests limit-exceeded, scheduler payload, voucher flow.
...
- Entry: Stage 6 green.
- Tasks: `statistics_definitions` CRUD (admin) + `services/stats_svc.py` resolvers (`most_used_template`, `last_fetch_by_user`, `beacon_success_rate`) + generic external fetcher (`httpx` with timeout + `required_params` validation); `GET /stats/{name}` permission-gated; tests internal + mocked external (respx/httpx mock) + forbidden without permission.
...
- **SQLAlchemy:** 2.0 typed ORM, async session per request, parameterized queries, migrations-ready (Alembic optional for PoC — `init_db` suffices), SQLite WAL, indexes on `username, identifier, (name,version), usage(template_id, requested_by), beacon(usage_id), logs(created_at, action)`.
...
1. Seed: roles/permissions, `admin/admin123`, `operator/operator123`, categories `python|json`, template `hello.py` (python), resource `ZY323S5GHW` + metadata dict, group `lab-phones` assigned to `operator` role, grants, `direct|scheduler|voucher` modes, processor `lab-runner` token, usage limit (e.g. 10/day), stats defs.
2. Flow: login both → operator `GET /templates/{id}/fetch` → `POST /usages {mode:voucher}` → `external_dispatch_id=V-xxx` → `POST /beacons` (processor token) → `GET /usages/{id}/status` + beacons → `GET /stats/most_used_template` → negative test (ungranted template → 403) → print activity logs.
...
- Terminology `template` everywhere; categories include `python` + `json`.
- RBAC + grants + groups gate template/resource access.
