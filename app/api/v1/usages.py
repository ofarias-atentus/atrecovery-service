"""Routine usages router: relay dispatch requests.

Routines never execute standalone: POST needs a ``resource_id`` plus
``routine:use`` + use grants on BOTH the routine and the resource
...
from app.services.activity import ROUTINE_USE, client_ip, log_activity
from app.services.usage_svc import create_usage, next_fire_at

router = APIRouter()
USE = require_permission("routine:use")
...
def _to_read(u: RoutineUsage, modes: dict[int, str]) -> UsageRead:
    return UsageRead(
        id=u.id,
...
async def _can_see(db: AsyncSession, user: User, u: RoutineUsage) -> bool:
    return u.requested_by == user.id or await _see_all(db, user)


@router.post(
    "", response_model=UsageRead, status_code=status.HTTP_201_CREATED, summary="Relay a routine usage"
)
async def post_usage(
    ...
    await log_activity(
        db, action=ROUTINE_USE, user_id=user.id,
        entity_type="usage", entity_id=usage.id,
        meta={"routine_id": usage.routine_id, "mode": body.mode, "resource_id": usage.resource_id},
...
async def list_usages(
    ...
    stmt = select(RoutineUsage).order_by(RoutineUsage.id.desc())
    if routine_id is not None:
        stmt = stmt.where(RoutineUsage.routine_id == routine_id)
    if await _see_all(db, user):
        if requested_by is not None:
            stmt = stmt.where(RoutineUsage.requested_by == requested_by)
    else:
        stmt = stmt.where(RoutineUsage.requested_by == user.id)
...
async def get_usage(
    ...
    u = (await db.execute(select(RoutineUsage).where(RoutineUsage.id == usage_id))).scalar_one_or_none()
    if u is None:
